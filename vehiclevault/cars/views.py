    ...
    if request.method == "POST":
        try:
            ...
            log_activity(
                request.user,
                "Price Updated",
                f"Price of '{car.brand} {car.model}' (VIN: {car.vin}) updated from ₹{old_price} → ₹{new_price}."
            )
            messages.success(request, f"Price updated to ₹{new_price:,.0f} successfully ✅")
        except (ValueError, TypeError):
            messages.error(request, "Invalid price entered. Please enter a valid number greater than 0. ❌")
    ...
    if request.method == "POST":
        try:
            ...
            log_activity(
                request.user,
                "Stock Updated",
                f"Stock of '{car.brand} {car.model}' (VIN: {car.vin}) updated from {old_stock} → {new_stock} units."
            )
            messages.success(request, f"Stock updated to {new_stock} units successfully ✅")
        except (ValueError, TypeError):
